...
export class SidebarComponent {
  ...
  ngOnInit(): void {
    ...
    this.dbService.subscribeToCollectionReactive(
      'users',
      (docs: UserData[]) => {
      ...
      this.dbService.maxDocs$
    ); // value for how much users a displayed
  }
  ...
  async createSubCollectionMessages(
  ...
  async checkIfPrivateChatExist(uid: string) {
    ...
    const privateChatQuery = query(
      chatRef,
      where('members', 'array-contains', this.dbService.userInformation.uid)
    );
    try {
      const privateChatSnapshot = await getDocs(privateChatQuery);
      const existingChat = await this.iterateOverPrivateChat(
        privateChatSnapshot,
        uid
      );
      if (existingChat) {
  ...
  async iterateOverPrivateChat(
    privateChatSnapshot: QuerySnapshot<DocumentData>,
    uid: string
  ) {
    for (const doc of privateChatSnapshot.docs) {
      ...
      if (
        ...
        members.includes(this.dbService.userInformation.uid)
      ) {
